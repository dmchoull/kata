...
describe CellAddedEvent do
  subject(:event) { CellAddedEvent.new(event_location) }
  let(:cell) { ::Cell.new(Location.new(1, 2), double(:@event_queue)) }

  describe '#trigger' do
    context 'inside the cell`s neighbourhood' do
      let(:event_location) { Location.new(2, 1) }

      it 'should increase the neighbour count of the cell' do
